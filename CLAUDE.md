...
- **Documentation is mandatory.** Every new feature, command, or API endpoint must include corresponding documentation in `docs/content/docs/`. When modifying existing features, update the relevant documentation pages. CLI changes go in `docs/content/docs/cli/`, ControlPlane changes in `docs/content/docs/control-plane/`, agent changes in `docs/content/docs/agent/`, Web UI changes in `docs/content/docs/web-ui/`. Do not ship a feature without docs.
