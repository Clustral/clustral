...
/// path uses <c>K8sStatusWriter</c> so that kubectl can natively render
/// the error. See the "Error Response Shapes" section in the root README
...
public static class ProblemDetailsWriter
{
        ...
        string? field = null)
    {
        ...
        {
            Type     = $"urn:clustral:error:{code.ToLowerInvariant()}",
    ...
    private static async Task WriteInternalAsync(HttpContext context, int statusCode, ProblemDetails problem)
    {
        ...
        if (!problem.Extensions.ContainsKey("traceId"))
            problem.Extensions["traceId"] = Activity.Current?.Id ?? correlationId;
